...
Say we want to study numerically its local maxima, and to do so we want to evaluate the function on two grids of different resolutions: This is a task that you will find yourself doing a lot of times when designing numerical simulations. For example, a grid with 10 values (in red), and another grid with 100 values (in black): 
```{r, echo=FALSE}
curve_plot + 
...
With currying, we can do so in a neat and elegant fashion: 
...
# overloading the arguments of seq to return 
# a lambda that takes an upper argument
def grid_factory(len):
    make_grid = lambda up: np.linspace(0, up, len)
    return make_grid
