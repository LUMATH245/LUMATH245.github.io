...
The equivalent reduce statement would be (note that this avoid the map statement, why?):
...
fibonacci_func <- function(prev, current) {
  n <- length(prev)
  c(prev, sum(prev[(n-1):n]))
}

# Generate the first 10 numbers of the Fibonacci series
reduce(rep(1, 8), fibonacci_func, .init = c(0, 1))
```
...
```{python}
# Define the function to be used in reduce
def fibonacci_func(prev, current):
    n = len(prev)
    return prev + [sum(prev[(n-2):n])]

# Generate the first 10 numbers of the Fibonacci series
reduce(fibonacci_func, [1]*8, [0, 1])
```
