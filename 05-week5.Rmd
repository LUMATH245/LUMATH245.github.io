...
R developers have created functions and packages for almost every
...
tasks, and knowing the right functions.
And R has changed a lot over the recent years, including more and more tools
for functional programming, fast data manipulation, exploration, analysis and plotting. In
particular, we will be working a lot with the `tidyverse` packages.

The `tidyverse` includes a wide range of packages. We have already seen one of those, 
in the past week the `purrr` library. In addition to that, other of the core packages in
...
manner. This makes the `tidyverse` a powerful tool for modern scientific computing, 
and for this reason I heavily encourage you to use its functions and packages for the rest of this entire module and for your future analyses.
...
packages you want to use at the start of every session. As a suggestion, make sure the `library(tidyverse)` command sits at the top of your script from now on.

In the previous chapters, we learned how to truly extract the power of functions. Well, one of the
things introduced in `purrr`, and that goes really well with the `tidyverse` package,
is the pipe operator. 
This operator allows you to pass the output of one function to another function as the
first argument. **This translates the mathematical function composition** in programming, e.g.  $(f_1 \circ f_2)(x)$. This operator is written as `|>` and is used to express
a sequence of multiple operations in a more readable and intuitive way. In code, the equivalent of our expression would be `x |> f1 |> f2`.

For example, if we want to compute the natural logarithm of the sum of
the first 10 natural numbers, $\ln(1 + 2+ 3+ ... + 10)$, this is:
...
In this example, we start with a vector `x` containing the numbers from
...
As you can see, using the pipe operator allows us to express the
...
However, we will also introduce a new package: `plotnine`. This package is not included in the standard Python library, so you'll need to install it. You can do this using pip, Python's package installer. To do so, type the following command:
...
`plotnine` is an implementation of a grammar of graphics in Python, based on `ggplot2` in R. `ggplot2` is one of the most used libraries for data visualization in R, and `plotnine` brings its powerful data visualization capabilities to Python. It's designed to work seamlessly with `pandas` data frames, making it extremely convenient for creating complex and beautiful statistical plots.
...
With `plotnine`, you'll be able to create a wide variety of plots and visualizations, and gain deeper insights from your data. So, let's dive in and start exploring the wonderful world of data visualization in Python! 🚀
...
To add new rows (or collate two data frames together!) we can use functions `bind_rows` (in R) and `concat` (in Python). This is formally referred to *concatenating*, which means to create a new data frame by attaching new rows to a data frame from a different one that shares the same columns. 
...
mtcars |>
  group_by(vs, am) |>
  summarise(avg_mpg = mean(mpg), avg_hp = mean(hp), min_qsec = min(qsec))
```
...
We can see now that the automatic transmission consume more in general, and accelerate in more time across both the types of engines.
Wait, what?
If you're an expert with cars, you probably heard that automatic consumes less (and accelerate faster in some sports cars!), so what's going on?
Well, remember, this data frame focuses on cars that were from 1973--74.
Back in the days the automatic transmission was mostly hydraulic, so it was quite inefficient!
While more recently we have much better transmissions, an expert driver can still be more fuel efficient and accelerate faster.
In F1, at this day, drivers still choose their gears on their own!

## Plotting with ggplot


The Grammar of Graphics is a framework for describing and building graphs. It was implemented in R through the `ggplot2` package and in Python through the `plotnine` package. The idea is to break up graphs into semantic components such as scales and layers so you can flexibly and systematically create a wide variety of plots.

### Basic Plots

Let's start with some basic plots using the `mtcars` dataset.

#### Histogram

A histogram is a representation of the distribution of a single variable, hence the `aes()` will only have the `x`. Let's create a histogram of `mpg` (miles per gallon).
...
A scatterplot uses two dimensions, allowing us to visualize the relationship between two variables. Let's create a scatterplot of `mpg` against `hp` (horsepower).
...
We can introduce higher dimensions by mapping additional variables to other aesthetics like color. Let's color the points in our scatterplot based on the type of transmission (`am`).
