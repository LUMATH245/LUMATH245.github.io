...
We have already added a couple of functions to our plots above.
Well, in ggplot you can change a bunch of graphical characteristics very easily by just adding things.
For example:
...
4.  **Limits**: You can set the limits of your axes using `xlim(lower, upper)` and `ylim(lower, upper)`.
...
Let's see what happens by adding the argument `nrow=3` to the `facet_wrap`, e.g. `facet_wrap(~gear, nrow=3)`.
