...
```{r}
# Create a sequence of numbers from -5 to 5 with increments of 0.1
x <- seq(-5, 5, by = 0.1)

# Compute the value of x^2 - x for each element of x and assign it to y
y <- x^2 - x
```
...
# Find the index of the element in y that has the maximum value
which.max(y)

# Find the index of the element in y that has the minimum value
which.min(y)

# Find the minimum value of y (equivalent to above, why?)
y[which.min(y)]

# Check which elements of y are less than 0 and return a logical vector
neg_index <- y < 0

# Find the indices of the elements in y that are less than 0
which(neg_index)
