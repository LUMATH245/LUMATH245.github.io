...
- Greater than (`>`): This operator returns `TRUE` if the left operand is larger than the right operand, and `FALSE` otherwise. For example, `3 > 2` returns `TRUE`, but `2 > 3` returns `FALSE`.
- Less than (`<`): This operator returns `TRUE` if the left operand is smaller than the right operand, and `FALSE` otherwise.
- Greater than or equal to (`>=`): This operator returns `TRUE` if the left operand is larger than or equal to the right operand, and `FALSE` otherwise. For example, `3 >= 2` and `3 >= 3` both return `TRUE`, but `2 >= 3` returns `FALSE`.
- Less than or equal to (`<=`): As above, this operator returns `TRUE` if the left operand is smaller than or equal to the right operand, and `FALSE` otherwise.

Note that the comparison is going to be consistent with the equality and inequality operators, meaning that if `x == y`, then `x >= y` and `x <= y` are both `TRUE`, and if `x != y`, then either `x > y` or `x < y` is `TRUE`.
...
direction in which the instructions of a program are executed. Flow can
be controlled by using conditional statements, such as if - else if -
else. Conditional statements allow us to execute different blocks of
code depending on the result of a logical expression or condition. For
example, we can use conditional statements to perform different actions
based on the value of a variable, the input of the user, or the outcome
of a calculation.
...
As you can see, the syntax and the output are very similar in both
languages, except that Python uses a colon and indentation to indicate
the start and the end of the block of code. Note that the if statement
only executes the block of code if the condition is true, and does
nothing otherwise.
...
As you can see, the syntax and the output are very similar in both
languages, except that R uses `else if` and Python uses `elif` for the
second condition. Note that the order of the conditions is important, as
only the first condition that is true is executed. If none of the
...
As you can see, the syntax and the output are very similar in both
languages, except that Python uses a colon and indentation to indicate
the start and the end of each block of code. Note that the nested if
statement checks the outer condition first, and then checks the inner
condition if the outer condition is true. Note also that the nested if
statement can have multiple levels of nesting, and can also include else
if and else blocks.
