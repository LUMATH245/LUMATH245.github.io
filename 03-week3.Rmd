...
**NOTE for Python users** In the print statement `print(f"{i} x {j} = {i*j}")` we used **f-string formatting**, which was introduced in Python 3.6 (the code will break on previous versions). It's a way to embed expressions (like `i*j`) inside strings, using curly braces `{}`. The expressions will be replaced with their values when the string is created. The letter `f` at the beginning of the string tells Python to allow these embedded expressions.
