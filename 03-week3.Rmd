...
We can wrap our code in a function, and every time the function is called, this code is run. This is incredibly useful for tasks we need to perform multiple times. Let’s look at an example of a simple function that converts cups to grams.
...
We'll create a new function to convert cups to milliliters. This function will take a second argument: "specific_weight". We'll use the function we created earlier to convert cups to grams, and then, using the "specific_weight", we'll convert grams to liters. We'll set the default value of "specific_weight" to be the specific weight of water.
...
By adding more arguments to our function, we can make it more flexible and powerful. We can now convert cups to milliliters for different substances, not just water! This is the power of functions in programming. 😊

- tell that we can have multiple arguments to a function
- show how to add more arguments to a function, to give more elements to the local
enviroment. 
- for instance, make a new function to convert cups to milliliters
  - the function should take a second argument: "specific_weight"
  - use the function above to convert cups to grams 
  - then, using the second argument, specific weight, convert grams to liters
  - this second argument should default to water
  - keep the R and python code side to side
  - give an example where you run the function with the specific weight of water and of oil
  
- give an explanation of what an enviroment is, and what is the global envirment and local
enviroment 
- give an example of that using the function above 
