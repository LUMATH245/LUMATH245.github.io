...
Having covered how to write a for cycle, we come back to our Fibonacci example:
...
- `fibonacci[i] <- fibonacci[i - 1] + fibonacci[i - 2]` is the code chunk that we want to repeat. It calculates the `i`th number in the Fibonacci sequence as the sum of the two preceding numbers and appends it to the `fibonacci` vector.
- `i` is the object that stores our current index. It gets updated at each step over the sequence.
...
- `fibonacci.append(fibonacci[i - 1] + fibonacci[i - 2])` is the code chunk that we want to repeat. It calculates the `i`th number in the Fibonacci sequence as the sum of the two preceding numbers and appends it to the `fibonacci` list using the `append()` method.
- `i` is the object that stores our current index. It gets updated at each step over the sequence.
