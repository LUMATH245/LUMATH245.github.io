...
The multiplication and division calculations are performed first (`*` and `/`), followed by addition and subtraction (`+` and `-`), going from left to right.[\^01-week0-3]
...
Now, our dear Prof. Sinsinius Snap, to access the spells again, all he needs to do, is to recall the labels he used, e.g. the variable names, and the archive will remind him of the content of the bottles:
...
The thing is, we can store the results of our calculations in an variable using the assignment operator, as mentioned above. For instance:
...
If you look in the environment panel of RStudio, you should now see two objects listed. These objects are now part of our working environment, and they refer to the variables that are accessible to the current R or Python session. We can access their contents by typing entering their names into the console:
...
When creating objects, remember R is case-sensitive so `x` is different from `X`:

```{r error=TRUE}
X
```

And if you decide to assign a new value to an object, R or python will not hesitate in overwriting it without warning:
...
Now the cool thing, whilist in maths something like $x = x + 1$ does not make much sense, in programming language it just works like magic. We can increase the value an object contains by referencing itself:
...
In fact, all mathematical operations can be applied like this.
