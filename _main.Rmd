...
programs. The precision of a floating-point number is determined by the number of significant digits that it can represent: double-precision floats, the defaults in R and python, can represent about 16 decimal digits of precision. Albeit systems are in place to avoid numerical errors for smaller numbers, **be careful when you compare two floats** and try to avoid comparing two floating point numbers to see if they are equal. Instead you could see if the discrepancy between them is less than some small, positive number (see later).
...
alter the flow of a loop. They can be incredibly useful in many
situations, such as when we want to stop a loop if a certain condition
...
We're about to create a weird overly-engineered version of FizzBuzz: `fizzbuzz_v10299384` (the vector solution we had in Chapter 2 was pretty elegant already, so use that in coding interviews).

Recall, that a function can take *another function* as input and return a new function by overloading its arguments. It gets pretty crazy what you can do with these things:
...
Clearly, what we have written in Week 2, as FizzBuzz, is a better solution (FizzBuzz is a very simple program), however this was a good example to illustrate the possibilities of functional programming. Soon, in the next exercise, we are going to use some of these concepts to build *the ultimate FizzBuzz program* (not underselling).

In fact, the ability to overload and return functions will be extremely useful when we introduce the `map` and `reduce` operations.
...
In this example, the list comprehension statement relies on the `zip` function. `zip` is used to pair up the elements from `numbers1` and `numbers2`. Try to run `zip(numbers1, numbers2)` alone to see the output! I personally find that the `map` function is more readable, however you should be familiar with both ways as you will be encountering both if you keep programming with Python.

**Shorter statements**

We can write the map2 statements above in a very compact way again:
...
[![](images/chapter4/reduce.png){width="265"}](reduce_scheme){#reduce_scheme}
...
### Exercise: Implement a Generic FizzBuzz Function
...
**NOTE for R users** Bored? This module is too easy? Well, R, in the recent years, has been undergoing one of the most beautiful transformations of the languages. In general, inexperienced R users tend to drift away from the language as it's seen as inelegant, ugly, or "hacky". Well, that was prior to the `ggplot2`, `purrr` and, more in general, `tidyverse` revolution. What if I tell you that you can code the above `generic_fizzbuzz` in under 4 statements (lines) without using any brackets `{}`? You will need the pipe operator `|>` (which we'll be seeing next week), and another couple of functions. You can learn about these functions quickly via the `purrr` cheatsheet, [which you can find here](https://rstudio.github.io/cheatsheets/html/purrr.html).
...
Let's illustrate how the recursion works with the same Fibonacci series example recursion. 
We can define a function called $f$ that: 
...
However, it's important to note that while recursion is a powerful concept, it can be impractical in practice due to its high computational complexity and the risk of running out of memory for large inputs. 
...
Sure, here's how you can expand on the concept of "reshaping a range" and illustrate the transpose operation:
...
```{r}
# Define a 3x3 full-rank matrix in R
...
```{python}
# Define a 3x3 full-rank matrix in Python
